...
export class CharacterRepository {
    ...
    async createCharacter(character: Character): Promise<number> {
    ...
    async getCharacterById(id: number): Promise<Character | null> {
        const [rows] = await this.db.query<Character[]>(`SELECT * FROM characters WHERE id = ?`, [id]);
        return rows[0] || null;
    }

    async updateCharacter(id: number, character: Partial<Character>): Promise<void> {
        await this.db.query(`UPDATE characters SET ? WHERE id = ?`, [character, id]);
    }

    async deleteCharacter(id: number): Promise<void> {
        await this.db.query(`DELETE FROM characters WHERE id = ?`, [id]);
    }

    async listCharactersByUser(userId: number): Promise<Character[]> {
        const [rows] = await this.db.query<Character[]>(`SELECT * FROM characters WHERE userid = ?`, [userId]);
        return rows;
    }
}
